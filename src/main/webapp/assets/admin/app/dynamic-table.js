	...
	a.widget("ui.dynamictable", {
		...
		_create: function() {
			...
			d.on("click", "a.btn-table-remove-line", function(k) {
				...
				var i = j.find("input[name$=\"extraAttributes['operation']\"]").val();
				if (i == "update") {
					j.find("input[name$=\"extraAttributes['operation']\"]").val("remove");
					j.hide();
			...
			if (b.length > 0) {
				b.each(function(k) {
					...
					l.find("input,select,textarea").each(function() {
						var i = a(this);
						var n = i.attr("id");
						if (n && !Util.startWith("__")) {
							i.removeAttr("id")
						}
					});
		...
		addLine: function(b, d) {
			...
			var f = a('<td style="text-align: center;" class="row-seq"><span class="span-row-seq">1</span></td>').prependTo(e);
		...
		resetIndex: function() {
			...
			$con.find("tbody tr").not("[row-index-fixed]").each(function() {
				a(this).find("input,select,textarea").each(function() {
					var b = a(this);
					if (b.attr("name")) {
						var c = b.attr("name");
						if (c) {
							b.attr("name", c.substring(0, c.indexOf("[") + 1) + idx + c.substring(c.indexOf("]"), c.length))
						}
						var e = b.attr("data-cascade-name");
						if (e) {
							b.attr("data-cascade-name", e.substring(0, e.indexOf("[") + 1) + idx + e.substring(e.indexOf("]"), e.length))
						}
						var f = b.attr("data-cascade-parent");
						if (f) {
							var d = f.substring(0, f.indexOf("[") + 1) + idx + f.substring(f.indexOf("]"), f.length);
							b.attr("data-cascade-parent", d)
						}
					}
					var g = b.attr("id");
					if (g && !Util.startWith("__")) {
						b.removeAttr("id")
					}
				});
				idx++
