	...
	return {
					...
					return this.each(function() {
						...
						$(d).keydown(function(h) {
							var l = $(d).find("tr[tabindex=0]")[0],
								n, g, m, f = d.p.treeReader.expanded_field;
							if (l) {
												...
												if (n.p.delurl == undefined || n.p.delurl == "clientArray") {
													$.each(q, function(s, v) {
														if (Util.startWith(v, "-")) {
															...
															for (var r in u) {
																if (r == "id" || Util.endWith(r, ".id") || r == "_arrayIndex") {} else {
		...
		initGrid: function(X, G) {
			...
			if (N.pager || N.toppager) {
				...
				ak.jqGrid("navSeparatorAdd", N.pager, {
					position: "first"
				});
