	...
	return {
		init: function() {
			$.fn.setFormDatas = function(data, forceOverwrite) {
				...
				Util.assertNotBlank(rules, "表单data-editrulesurl属性未定义");
				var url = WEB_ROOT + "/admin/util/validate/unique?clazz=" + rules.split("clazz=")[1] + "&element=" + $(element).attr("name");
		...
		initAjax: function($container) {
			...
			$("form.form-validation", $container).each(function() {
				...
				var dataDivSearch = $form.attr("data-div-search");
				$form.find('[type="reset"]').click(function() {
					if (dataGridSearch) {
						$(dataGridSearch).each(function() {
							$(this)[0].clearToolbar(false)
						})
					}
					setTimeout(function() {
						$form.find('[data-toggle="buttons"] label.btn > input.toggle').each(function() {
							if ($(this).is(":checked")) {
						...
						$form.find("input[data-date-init]").each(function() {
							var $datapicker = $(this);
							var dataPicker = $datapicker.attr("data-picker");
						...
						if (dataGridSearch || dataDivSearch) {
							$form.submit()
						}
					}, 100);
					return
				...
				var validator = $form.validate({
					...
					errorPlacement: function(error, element) {
						var $errorPlacement = element.closest(".form-group").find(".error-placement");
						if ($errorPlacement.size() > 0) {
							...
							if (element.parent(".input-group").size() > 0) {
								...
								if (element.parent(".input-icon").parent(".input-group").size()) {
									error.insertAfter(element.parent(".input-icon").parent(".input-group"))
								} else {
									if (element.attr("data-error-container")) {
										error.appendTo($(element.attr("data-error-container")))
									} else {
										if (element.parents(".controls-radiobuttons").size() > 0) {
											error.appendTo(element.parents(".controls-radiobuttons"))
										} else {
											if (element.parents(".radio-list").size() > 0) {
												...
												if (element.parent(".radio-inline").size() > 0) {
													...
													if (element.parents(".controls-checkboxes").size() > 0) {
														error.appendTo(element.parents(".controls-checkboxes"))
													} else {
														if (element.parents(".checkbox-list").size() > 0) {
															...
															if (element.parent(".checkbox-inline").size() > 0) {
																...
																if (element.parent(".check-radio-label").size() > 0) {
																	error.appendTo(element.parent(".check-radio-label").parent())
																} else {
					...
					invalidHandler: function(event, validator) {
						App.scrollTo($(validator.errorList[0].element).closest(".form-group"), -100)
					},
					...
					unhighlight: function(element) {
						var $errorPlacement = $(element).closest(".form-group").find(".error-placement");
						if ($errorPlacement.size() > 0) {
							if ($errorPlacement.find(".error-block").size() > 0) {
								return
							}
						}
						$(element).closest("td").removeClass("has-error");
						$(element).closest(".form-group").removeClass("has-error");
						$(element).closest("form").find(".form-error-placement").empty()
					},
					...
					submitHandler: function(form) {
						...
						var ret = $form.trigger("form-validate-success");
						...
						if (dataDivSearch) {
							var $container = $form.closest(".panel-content");
							if ($container.size() == 0) {
								$container = $("body")
							}
							var $target = $container.find(dataDivSearch);
							if ($target.is("tbody")) {
								$target.parent("table.table-infinite-scroll").removeAttr("data-scroll-loading").removeAttr("data-scroll-page")
							}
						...
						var $actions = submitButton.closest(".form-actions");
						if ($actions.size() == 0) {
							$actions = submitButton
						}
						$form.find(".alert-validation").remove();
						...
						$form.ajaxSubmit({
							...
							success: function(response) {
								...
								if (response.type == "confirm") {
									bootbox.dialog({
										closeButton: false,
										message: response.data ? response.data.join("<br/>") : response.message,
								...
								if (response.type == "success" || response.type == "warning") {
									var result = $form.triggerHandler("form-submit-success", [response]);
									if (result != undefined && result == false) {
										return
									}
									if (response.message) {
									...
									if (response.redirect) {
										window.location.href = WEB_ROOT + response.redirect
									} else {
										if (typeof(AdminPage) == "undefined") {
											var $alert = $form.find("div.alert-edit-success");
											if ($alert.size() == 0) {
												$alert = $("<div class='alert alert-success alert-edit-success'>" + response.message + "</div>");
												var $formActions = submitButton.closest(".form-actions");
												if ($formActions.size() > 0) {
													$alert.insertBefore($formActions)
												} else {
									...
									var dismiss = submitButton.attr("data-post-dismiss");
									if (dismiss) {
										var $modal = submitButton.closest(".modal-dialog");
										if (dismiss == "modal" && $modal.size() > 0) {
											$modal.find(".modal-header button[data-dismiss='modal']").click()
										}
									} else {
										var reload = submitButton.attr("data-success-reload");
										if (reload != "false") {
											var $modal = submitButton.closest(".modal");
											if ($modal.size() > 0) {
												var $tabbable = submitButton.closest(".tabbable", $modal);
												if ($tabbable.size() == 0) {
													$modal.modal("hide")
												} else {
													var $navItems = $tabbable.first().find("ul.nav > li:not(.tools)");
													if ($navItems.size() <= 1) {
														$modal.modal("hide")
													} else {
														var id = submitButton.closest("form").find("input[name='id']").val();
														if (id && id != "") {
															$tabbable.find(" > ul.nav > li.tools > .reload").click()
														} else {
															var $tabbableContainer = $tabbable.closest(".ajax-get-container");
															var url = Util.AddOrReplaceUrlParameter($tabbableContainer.attr("data-url"), "id", response.data.id);
															$tabbableContainer.ajaxGetUrl(url)
														}
													}
												}
											}
											if (reload == "_panel") {
												var $panelContent = submitButton.closest(".panel-content");
												$panelContent.ajaxGetUrl($panelContent.attr("data-url"))
											}
										}
									}
									if (dataGridReload) {
									...
									if (response.type == "failure" || response.type == "error") {
										var ret = $form.triggerHandler("form-submit-failure", [response]);
										if (ret == undefined || ret == true) {
											var $formErrorPlacement = $form.find(".form-error-placement");
											if ($formErrorPlacement.size() > 0) {
												$formErrorPlacement.closest(".form-group").addClass("has-error");
												$formErrorPlacement.html('<span class="error-block">' + response.message + "</span>")
											} else {
												$actions.before("<div class='alert alert-danger alert-validation'>" + response.message + "</div>")
											}
											$("img.captcha-img", $form).click()
										}
									} else {
										$actions.before("<div class='alert alert-danger alert-validation'>E03: 表单处理异常，请联系管理员</div>");
										$("img.captcha-img", $form).click()
									}
								}
							},
							error: function(xhr, e, status) {
								...
								$actions.before("<div class='alert alert-danger alert-validation'>" + message + "</div>")
							}
						});
						return false
					}
